...
import type { KsListing } from "@/lib/demoApi";

interface KeystoneClientProps {
  /** Live listings from the Keystone API. */
  listings: KsListing[];
}
...
function specsFor(l: KsListing): string[] {
  return [`${l.beds} bed`, `${l.baths} bath`, `${l.sqft.toLocaleString()} sqft`];
}

function whyFor(l: KsListing): string | undefined {
  // The seed includes a transit note for many listings — surface it as
  // the "why" line so the card has real context beyond the bare specs.
  return l.transitNote || (l.nearTransit ? "Near transit" : undefined);
}
...
// Listings now come from the live Keystone API via the page's props.
...
export default function KeystoneClient({ listings }: KeystoneClientProps) {
    ...
    >
      ...
      <div className="grid lg:grid-cols-[1fr_480px] grid-cols-1 flex-1" style={{ height: "calc(100vh - 130px)", minHeight: 560 }}>
        ...
        <div className="bg-white lg:border-l border-t lg:border-t-0 border-[#E1DCCF] overflow-y-auto px-6 py-4.5">
          ...
          {listings.length === 0 ? (
            <div className="py-10 text-center text-[#5E6B78] text-[14px]">
              No active listings right now — ask Buddy what&apos;s coming on the market.
            </div>
          ) : null}
          {listings.map((l, i) => {
            const why = whyFor(l);
            return (
              <Link
                key={l.listingId}
                href={`/demo/keystone/listing/${l.listingId}`}
                className={`grid gap-3.5 py-3.5 cursor-pointer no-underline text-inherit ${i < listings.length - 1 ? "border-b border-[#E1DCCF]" : ""} hover:bg-[rgba(31,77,62,0.03)]`}
                style={{ gridTemplateColumns: "140px 1fr" }}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={l.image || "https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=600&q=70"}
                  alt={l.address}
                ...
                <div>
                  ...
                  <span
                    className="inline-block px-1.5 py-0.5 text-white text-[10px] tracking-[0.04em] uppercase mt-1.5 font-semibold rounded-sm capitalize"
