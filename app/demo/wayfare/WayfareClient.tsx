...
import type { WfPackage, WfStay, WfDestination } from "@/lib/demoApi";

interface WayfareClientProps {
  /** Real curated trip packages fetched from the live Wayfare API. */
  packages: WfPackage[];
  /** Top stays for the featured destination — drives the sidebar's "Top stays". */
  stays: WfStay[];
  /** All destinations — used to resolve the featured destination's name in the top bar. */
  destinations: WfDestination[];
  /** Code of the destination featured in the top bar (defaults to first package's). */
  featuredCode: string;
}
...
// RECENT_TRIPS + SAVED_PLACES come from the live Wayfare API now —
// see WayfareClientProps above. Helpers below format the API shapes for
// the sidebar's title + subtitle rows.
...
function packageSubtitle(pkg: WfPackage): string {
  const next = pkg.departures?.[0];
  const where = pkg.destinationCode;
  if (pkg.seatsLeft === 0) return `Sold out · ${where}`;
  if (pkg.seatsLeft <= 3) return `${pkg.seatsLeft} seats left · departs ${formatDeparture(next)}`;
  return `From $${pkg.priceFromUsd} · departs ${formatDeparture(next)}`;
}

function stayTier(rating: number): string {
  return rating >= 4.7 ? "★★★★★" : rating >= 4.3 ? "★★★★" : "★★★";
}
...
const PLACES: Place[] = [
  { match: ["memmo alfama"], name: "Memmo Alfama Hotel", tag: "Hotel", sub: "Alfama · 4★", img: "https://live.staticflickr.com/8578/16474476320_e97ec53c4d_b.jpg" },
  { match: ["casa do bairro"], name: "Casa do Bairro", tag: "Guesthouse", sub: "Graça · $95/night", img: "https://live.staticflickr.com/65535/48495444347_09324f3300_b.jpg" },
  { match: ["hotel sotão", "hotel sotao"], name: "Hotel Sotão", tag: "Hotel", sub: "Bairro Alto · 4★", img: "https://live.staticflickr.com/4146/4955580943_801634b39d_b.jpg" },
  { match: ["pois cafe", "pois café"], name: "Pois Café", tag: "Café", sub: "Alfama · €", img: "https://live.staticflickr.com/8369/8590538812_f39d874e2c_b.jpg" },
  { match: ["taberna da rua das flores"], name: "Taberna da Rua das Flores", tag: "Restaurant", sub: "Chiado · €€", img: "https://live.staticflickr.com/2733/5833827591_b45a599d30_b.jpg" },
  { match: ["casamento"], name: "Casamento", tag: "Restaurant", sub: "Seafood rice · €€", img: "https://live.staticflickr.com/1032/1158217645_b7181246ff_b.jpg" },
  { match: ["cervejaria patrícia", "cervejaria patricia"], name: "Cervejaria Patrícia", tag: "Restaurant", sub: "Clams · €€", img: "https://live.staticflickr.com/7201/6792509022_f59096317d_b.jpg" },
  { match: ["time out market"], name: "Time Out Market", tag: "Food hall", sub: "Cais do Sodré · €€", img: "https://live.staticflickr.com/65535/52231761318_7a7f534c20_b.jpg" },
  { match: ["pastéis de belém", "pasteis de belem"], name: "Pastéis de Belém", tag: "Bakery", sub: "Belém · since 1837", img: "https://live.staticflickr.com/65535/51912566340_f3eca64ff2_b.jpg" },
  { match: ["jerónimos monastery", "jeronimos monastery"], name: "Jerónimos Monastery", tag: "Landmark", sub: "Belém · UNESCO", img: "https://live.staticflickr.com/3029/2380261504_18289d0c19_b.jpg" },
  { match: ["belém tower", "belem tower"], name: "Belém Tower", tag: "Landmark", sub: "Belém · 16th c.", img: "https://live.staticflickr.com/5695/22447126166_2db1545ed4_b.jpg" },
  { match: ["são jorge castle", "sao jorge castle"], name: "São Jorge Castle", tag: "Landmark", sub: "Alfama · hilltop", img: "https://live.staticflickr.com/4181/34403843531_73673b942f_b.jpg" },
  { match: ["lx factory"], name: "LX Factory", tag: "District", sub: "Alcântara · art + shops", img: "https://live.staticflickr.com/2665/5833997190_3ac5f31652_b.jpg" },
  { match: ["costa da caparica"], name: "Costa da Caparica", tag: "Beach", sub: "45 min by bus", img: "https://live.staticflickr.com/8432/7840746018_1df281cb80_m.jpg" },
  { match: ["cha-cha", "chiringuito"], name: "Cha-Cha Beach Bar", tag: "Beach bar", sub: "Caparica · chiringuito", img: "https://live.staticflickr.com/7807/39681042993_097d45a2b5_b.jpg" },
  { match: ["miradouro da senhora do monte", "senhora do monte"], name: "Miradouro Senhora do Monte", tag: "Viewpoint", sub: "Graça · sunset", img: "https://live.staticflickr.com/17/21466314_cc34794b73_b.jpg" },
  { match: ["príncipe real", "principe real"], name: "Príncipe Real", tag: "Neighborhood", sub: "Younger, artsier", img: "https://live.staticflickr.com/2567/3762063327_3105d4430a_b.jpg" },
  { match: ["sintra"], name: "Sintra", tag: "Day trip", sub: "30 min by train", img: "https://live.staticflickr.com/2244/2511532700_57ef637015_b.jpg" },
  { match: ["pena palace"], name: "Pena Palace", tag: "Palace", sub: "Sintra · ridge-top", img: "https://live.staticflickr.com/65535/48806840888_48184378d5_b.jpg" },
  { match: ["quinta da regaleira"], name: "Quinta da Regaleira", tag: "Garden", sub: "Sintra · wells + grottoes", img: "https://live.staticflickr.com/1410/869441178_5f92b8a6ce_b.jpg" },
  { match: ["alfama"], name: "Alfama", tag: "District", sub: "Old quarter", img: "https://live.staticflickr.com/65535/47745943682_d908c7b48b_b.jpg" },
  { match: ["belém", "belem"], name: "Belém", tag: "District", sub: "Riverside · monuments", img: "https://live.staticflickr.com/4201/34800208366_9e2b155056_b.jpg" },
];
...
export default function WayfareClient({
  packages,
  stays,
  destinations,
  featuredCode,
}: WayfareClientProps) {
  const featured = destinations.find((d) => d.code === featuredCode) || destinations[0];
  const sidebarPackages = packages.slice(0, 5);
  const sidebarStays = stays.slice(0, 4);
  ...
  return (
    ...
    >
      ...
      <aside className="hidden md:flex bg-[#1A1A1A] text-white p-4.5 flex-col gap-3.5 overflow-y-auto">
        ...
        <div className="text-[10.5px] tracking-[0.14em] uppercase text-[#7A736A] px-2 pt-3.5 pb-1.5">Trip packages</div>
        {sidebarPackages.length === 0 ? (
          <div className="px-3 py-2.5 text-[12px] text-white/45">No packages right now.</div>
        ) : (
          sidebarPackages.map((p, i) => (
            <Link
              key={p.packageId}
              href={`/demo/wayfare/destination/${p.destinationCode.toLowerCase()}`}
              className={`px-3 py-2.5 rounded-lg text-[13px] text-white/85 cursor-pointer flex flex-col gap-0.5 no-underline ${
                i === 0 ? "bg-white/[0.10]" : "hover:bg-white/[0.06]"
              }`}
            >
              <span className="font-medium">{p.title}</span>
              <small className="text-[11px] text-white/50">{packageSubtitle(p)}</small>
            </Link>
          ))
        )}

        <div className="text-[10.5px] tracking-[0.14em] uppercase text-[#7A736A] px-2 pt-3.5 pb-1.5">
          Top stays{featured ? ` · ${featured.name}` : ""}
        </div>
        {sidebarStays.length === 0 ? (
          <div className="px-3 py-2.5 text-[12px] text-white/45">Loading availability…</div>
        ) : (
          sidebarStays.map((s) => (
            <div
              key={s.stayId}
              className="px-3 py-2.5 rounded-lg text-[13px] text-white/85 hover:bg-white/[0.06] cursor-pointer flex flex-col gap-0.5"
            >
              <span className="font-medium">{s.name}</span>
              <small className="text-[11px] text-white/50">
                ${s.pricePerNightUsd}/night · {stayTier(s.rating)} · {s.type}
              </small>
            </div>
          ))
        )}
      ...
      <section className="flex flex-col h-screen overflow-hidden">
        <div className="px-7 py-3.5 border-b border-[#E5DFD3] bg-[#F8F5F0] flex items-center gap-3.5">
          ...
          <span className="text-[18px] font-semibold ml-1" style={{ fontFamily: "var(--font-bricolage), sans-serif" }}>
            {sidebarPackages[0]?.title || (featured ? featured.name : "Wayfare")}
          </span>
